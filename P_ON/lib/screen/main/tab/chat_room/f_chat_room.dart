...
class _ChatRoomState extends ConsumerState<ChatRoom> {
  ...
  Future<void> sendChatLog() async {
    ...
    try {
      Response response = await apiService.sendRequest(
          method: 'PUT',
          path:
              '$server/api/promise/chat/${messages.last['roomId']}/${messages.last['id']}',
          headers: headers);
      print('마지막 채팅 기록 저장');
    } catch (e) {
      print(e);
  ...
  Future<void> getChat() async {
    ...
    try {
      Response response = await apiService.sendRequest(
          method: 'GET',
          path: '$server/api/promise/chat/${widget.id}',
          headers: headers);
      setState(() {
        print('여기실행');
        messages.add({'today': '이전채팅을 받아왔습니다'});
        final chat = response.data['result'][0];
        messages.addAll(chat.cast<Map<String, dynamic>>());
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            leading: IconButton(
              ...
              onPressed: () async {
                if (messages.last.containsKey('roomId')) {
                  await sendChatLog();
                }
                context.go('/main');
              },
                                              ...
                                              child: Container(
                                                ...
                                                decoration: !isCurrentUser
                                                    ...
                                                    : BoxDecoration(
                                                        color:
                                                            AppColors.mainBlue2,
                                                        borderRadius: isLastMessageFromSameSender
                                                            ? const BorderRadius
                                                                .only(
                                                                topLeft: Radius
                                                                    .circular(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: CircleAvatar(
                    backgroundColor: AppColors.mainBlue50,
                    radius: 32,
                    child: Image.asset(
                        ...
                        child: Row(
                          children: [
                            Expanded(
                              ...
                              onSubmitted: (text) {
                                if (text.isNotEmpty) {
                                  sendChat(widget.id, userId, text);
